...
const DiaryScreen = () => {
    ...
    const [parsedEvent, setParsedEvent] = useState(event ? JSON.parse(event as string) : null);
    const [match, setMatch] = useState(parsedEvent?.Title ?? null);
    const [matchContent, setMatchContent] = useState({
        ...
        weather: parsedEvent?.Weather ?? "",
        mood: parsedEvent?.Mood ?? "",
